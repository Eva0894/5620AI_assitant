...
const OthersReport = () => {
  ...
  useEffect(() => {
    ...
    const fetchOthersReport = async () => {
      const { data, error } = await supabase
        .from('report')
        .select('*')
        .eq('elder_id', elderId)
        .eq('type', 'others');  // 筛选其他报告类型
  ...
  return (
    <div style={styles.container}>
      <h1 style={styles.title}>Other Report for Elder {elderName}</h1>
      <div style={styles.reportContainer}>
        {report ? (
          <table style={styles.table}>
            <thead>
              <tr>
                <th style={styles.tableHeader}>Other Report</th>
              </tr>
            </thead>
            <tbody>
              <tr>
